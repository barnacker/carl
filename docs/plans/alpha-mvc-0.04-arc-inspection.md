...
- ArcState vocabulary is derived: `ENGAGED`, `INCUBATING`, `INHIBITED`, `RESOLVED`, `ABSORBED`. Trace events carry lifecycle evidence such as `ARC_OPEN`, `ARC_ACTIVE`, and `ARC_RESOLVED`.
...
- adds states outside `OPEN`, `ACTIVE`, `ENGAGED`, `INCUBATING`, `INHIBITED`, `RESOLVED`, `ABSORBED`;
